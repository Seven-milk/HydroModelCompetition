...
class Model02(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm_pre = torch.nn.LSTM(input_size=20, hidden_size=1, batch_first=True, num_layers=1)
        self.lstm_runoff = torch.nn.LSTM(input_size=4, hidden_size=16, batch_first=True, num_layers=1)
        self.linear = torch.nn.Linear(in_features=32, out_features=16, bias=True)
        self.activate = torch.nn.Sigmoid()

    def forward(self, runoff, pre):
        out_pre, _ = self.lstm_pre(pre)
        out_pre = out_pre[:, -16:, :]
        _, (hidden_runoff, _) = self.lstm_runoff(runoff)
