...
class Model01(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn_pre = torch.nn.RNN(input_size=20, hidden_size=1, batch_first=True)
        self.rnn_runoff = torch.nn.RNN(input_size=4, hidden_size=16, batch_first=True)
        self.linear = torch.nn.Linear(in_features=32, out_features=16, bias=True)
        self.activate = torch.nn.Sigmoid()

    def forward(self, runoff, pre):
        out_pre, _ = self.rnn_pre(pre)
        out_pre = out_pre[:, -16:, :]
        _, hidden_runoff = self.rnn_runoff(runoff)
...
def train_cycle_model(epochs=100):
    ...
    # checkpoint
    path_checkpoint_read = "H:/文件/水科学数值模拟/复赛/模型/RNN/checkpoint/ckpt_best_model01.pkl"
    ...
    for epoch in list(range(start_epoch + 1, start_epoch + 2 + epochs)):
        ...
        if (epoch % 50 == 0) or (len(loss_list) > 2 and loss_list[-2] - loss_list[-1] / loss_list[-2] > 5):
            ...
            torch.save(checkpoint, f"H:/文件/水科学数值模拟/复赛/模型/RNN/checkpoint/ckpt_best_model01_{epoch}.pkl")
    ...
    torch.save(checkpoint, "H:/文件/水科学数值模拟/复赛/模型/RNN/checkpoint/ckpt_best_model01.pkl")
    torch.save(model, 'H:/文件/水科学数值模拟/复赛/模型/RNN/model01.pth')


# model overview
def overview_model():
    checkpoint = torch.load('H:/文件/水科学数值模拟/复赛/模型/RNN/checkpoint/ckpt_best_model01.pkl')
    epoch_list = checkpoint["epoch_list"]
    loss_list = checkpoint["loss_list"]
    nse_list = checkpoint["nse_list"]
...
def predict(runoff, pre):
    ...
    '''
    model = torch.load('H:/文件/水科学数值模拟/复赛/模型/RNN/model01.pth')
